...
function insertAnswer($questionId, $userId, $title, $content) {
    $conn = dbcon();
    if (!$conn) {
    ...
    $stmt = $conn->prepare("INSERT INTO answer (userid, questionid, title, content) VALUES (?, ?, ?, ?)");
    if (!$stmt) {
        echo "Error: " . $conn->error;
    ...
    $stmt->bind_param("ssss", $userId, $questionId, $title, $content);
    $result = $stmt->execute();
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $title = $_POST['answer_title']; // Get the answer title from the form
    $content = $_POST['answer_content'];

    // Insert the answer into the database
    $success = insertAnswer($questionId, $userId, $title, $content);
    ...
    if($question) {
        ...
        <body>
            <div class="container">
                ...
                <form method="post" action="create_answer.php?id=<?php echo $questionId; ?>">
                    <!-- Include a textbox for the answer title -->
                    <input type="text" name="answer_title" placeholder="Enter your answer title" required><br>
                    <!-- Include any necessary form fields for the answer -->
                    <textarea name="answer_content" rows="4" cols="50" placeholder="Type your answer here..." required></textarea><br>
