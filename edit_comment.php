...
// Fetch question data
$comment = get_TheComment($comment_id);
if (empty($comment)) {
...
function get_TheComment($comment_id) {
    $db = dbcon();
    $stmt = $db->prepare("SELECT * FROM comment_answer WHERE id = ?");
...
<html lang="en">
    ...
    <body>
        <div class="container">
            ...
            <form action="process_comment_update.php" method="post">
                <input type="hidden" name="comment_id" value="<?php echo $comment['id']; ?>">
                <input type="hidden" name="answer_id" value="<?php echo $answer_id; ?>">
                <textarea name="content" rows="6" required><?php echo htmlspecialchars($comment['content']); ?></textarea>
